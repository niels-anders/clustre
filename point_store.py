...
def save(shapePath, geoLocations, proj4):
    ...
    #geoLocations = np.float32(geoLocations)    
    ...
    # add field z
    new_field = osgeo.ogr.FieldDefn('z',osgeo.ogr.OFTReal)
    ...
    for pointIndex, geoLocation in enumerate(geoLocations):
        ...
        geometry.SetPoint(0, geoLocation[0], geoLocation[1])#, geoLocation[2])
    
        # Create feature
        feature = osgeo.ogr.Feature(layerDefinition)
        feature.SetGeometry(geometry)
        feature.SetFID(pointIndex)
        #feature.SetField('z',geoLocation[2])
        # Save feature
